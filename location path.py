...
# Step 1: Location data (name, latitude, longitude)
locations = {
    ...
    "Main Gate": [31.39415791790809, 75.53297552271245],
    "Dispensary NITJ": [31.394209555472536, 75.53757522247375],
    "Nescafe": [31.394212454423144, 75.53707045450096],
    "Megaa Girls Hostel Phase 2": [31.39433710258572, 75.53986666560951],
...
# Step 2: Haversine distance function (in meters)
...
for name1, coord1 in locations.items():
    for name2, coord2 in locations.items():
        if name1 != name2:
            ...
            if distance < 200:  # Connect only nearby locations (threshold)
                G.add_edge(name1, name2, weight=distance)

# Step 4: Define start and end locations
start = "NITJ Temple"
end = "Mega Boys Hostel F"

# Step 5: Find shortest path
shortest_path = nx.dijkstra_path(G, source=start, target=end, weight='weight')
print("Shortest Path:", shortest_path)

# Step 6: Visualize path on map
m = folium.Map(location=locations[start], zoom_start=17)
...
# Draw the path
path_coords = [locations[name] for name in shortest_path]
folium.PolyLine(path_coords, color="red", weight=5, opacity=0.8).add_to(m)

# Save and show
m.save("nitj_path_map.html")
print("Map saved as 'nitj_path_map.html'")
